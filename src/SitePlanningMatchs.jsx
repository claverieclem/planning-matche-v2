  ...
  return rows
    ...
    .map((row) => {
      ...
      return {
        ...
        locationType: isHome ? 'domicile' : isAway ? 'exterieur' : cleanText(row[venueIdx]).toLowerCase(),
        place: locationIdx >= 0 ? cleanText(row[locationIdx]) : '',
      };
    })
    .filter((match) => DAY_NAMES.some((day) => match.day.includes(day)) && match.team && match.time);
}
...
function groupByDay(matches) {
  return DAY_NAMES.map((day) => ({
    day,
    items: matches.filter((match) => match.day.includes(day)),
...
function Section({ title, groups, home }) {
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <h2 className="text-2xl font-bold tracking-tight">{title}</h2>
        <Badge variant="secondary">{groups.reduce((acc, group) => acc + group.items.length, 0)} match(s)</Badge>
      </div>

      {groups.map((group) => (
        <Card key={group.day} className="rounded-2xl shadow-sm">
          <CardHeader>
            <CardTitle className="text-xl">{group.day}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="overflow-hidden rounded-2xl border border-red-100">
              <table className="w-full text-sm">
                <thead className="bg-muted/60">
                  <tr>
                    <th className="px-4 py-3 text-left">{home ? 'Heure' : 'Catégorie / équipe'}</th>
                    <th className="px-4 py-3 text-left">{home ? 'Catégorie / équipe' : 'Heure'}</th>
                    <th className="px-4 py-3 text-left">Adversaire / lieu</th>
                  </tr>
                </thead>
                <tbody>
                  {group.items.map((match, index) => (
                    <tr key={`${group.day}-${index}`} className="border-t">
                      <td className="px-4 py-3 font-medium">{home ? match.time : match.team}</td>
                      <td className="px-4 py-3">{home ? match.team : match.time}</td>
                      <td className="px-4 py-3 text-muted-foreground">{match.opponent || match.place || '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function PreviewMatchRow({ match, home }) {
  return (
    <div
      className={`grid grid-cols-[1.5fr_70px_1.3fr_130px] items-center rounded-full shadow-lg ${
        home ? 'bg-amber-400 text-black' : 'bg-sky-500 text-white'
      }`}
    >
      <div className="truncate px-6 py-4 text-center text-lg font-extrabold uppercase">{match.team}</div>
      <div className={`text-center text-2xl font-black ${home ? '' : 'text-yellow-200'}`}>VS</div>
      <div className="truncate px-4 py-4 text-center text-lg font-extrabold uppercase">
        {match.opponent || match.place || (home ? 'DOMICILE' : 'EXTÉRIEUR')}
      </div>
      <div
        className={`rounded-r-full border-l-4 border-red-800 px-4 py-4 text-center text-xl font-black ${
...
export default function SitePlanningMatchs() {
  ...
  const homeGroups = useMemo(() => groupByDay(homeMatches), [homeMatches]);
  const awayGroups = useMemo(() => groupByDay(awayMatches), [awayMatches]);
  ...
  DAY_NAMES.forEach((day) => {
    const homeGroup = homeGroups.find((group) => group.day === day);
    const awayGroup = awayGroups.find((group) => group.day === day);

    if (showHome && homeGroup) {
      slides.push({ type: 'matches', day, venue: 'Domicile', home: true, items: homeGroup.items });
    }
    if (showAway && awayGroup) {
      slides.push({ type: 'matches', day, venue: 'Extérieur', home: false, items: awayGroup.items });
    }
  });
      ...
      <div className="mx-auto max-w-7xl space-y-8 p-6 md:p-10">
        <div className="grid items-start gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="space-y-4">
            <Badge className="rounded-full bg-red-600 px-3 py-1 text-xs text-white shadow-sm">Planning handball</Badge>
            <h1 className="text-4xl font-black tracking-tight text-red-700 md:text-5xl">Site planning matchs</h1>
          ...
          <div className="space-y-6">
            <Card className="rounded-2xl border border-red-100 bg-white/95 shadow-md backdrop-blur">
              <CardHeader>
              ...
              <CardContent className="space-y-4">
                <label className="block">
                  <div className="flex min-h-36 cursor-pointer flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-red-300 bg-white/70 p-6 text-center transition-colors hover:bg-red-50">
                ...
                {fileName ? (
                  <div className="rounded-xl border p-3 text-sm">
                    <span className="font-medium">Fichier chargé :</span> {fileName}
                  </div>
                ) : null}

                {backgroundName ? (
                  <div className="rounded-xl border p-3 text-sm">
                    <span className="font-medium">Fond chargé :</span> {backgroundName}
                  </div>
                ) : null}

                {introName ? (
                  <div className="rounded-xl border p-3 text-sm">
                    <span className="font-medium">Page intro :</span> {introName}
                  </div>
                ) : null}
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <Button onClick={() => window.print()} disabled={!matches.length && !introImage} className="rounded-xl bg-red-600 text-white hover:bg-red-700">
                    <Printer className="mr-2 h-4 w-4" /> Imprimer / PDF
                  </Button>

                  <Button variant="outline" onClick={resetAll} className="rounded-xl border-red-300 text-red-600 hover:bg-red-50">
            ...
            <Card className="rounded-2xl border border-red-100 bg-white/95 shadow-md backdrop-blur">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-xl">
                  <Eye className="h-5 w-5" /> Mode aperçu comme Canva
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="flex flex-wrap gap-3">
                  <Button variant={showHome ? 'default' : 'outline'} className={showHome ? 'rounded-xl bg-red-600 text-white hover:bg-red-700' : 'rounded-xl border-red-300 text-red-600 hover:bg-red-50'} onClick={() => setShowHome((value) => !value)}>
                    {showHome ? 'Domicile affiché' : 'Afficher domicile'}
                  </Button>

                  <Button variant={showAway ? 'default' : 'outline'} className={showAway ? 'rounded-xl bg-blue-600 text-white hover:bg-blue-700' : 'rounded-xl border-blue-300 text-blue-600 hover:bg-blue-50'} onClick={() => setShowAway((value) => !value)}>
                    {showAway ? 'Extérieur affiché' : 'Afficher extérieur'}
                  </Button>
                </div>

                <div className="text-sm text-muted-foreground">
                  Tu peux maintenant regarder les pages une par une, comme un post Instagram.
                </div>

                <div className="flex items-center gap-3 text-sm text-muted-foreground">
                  <Button type="button" variant="outline" className="rounded-xl border-red-200 bg-white/80 text-red-700 hover:bg-red-50" onClick={goPrevSlide} disabled={safeSlideIndex === 0 || !slides.length}>
                    <ChevronLeft className="mr-2 h-4 w-4" /> Précédent
                  </Button>
                  <div>
                    Slide {slides.length ? safeSlideIndex + 1 : 0} / {slides.length}
                  </div>
                  <Button type="button" variant="outline" className="rounded-xl border-red-200 bg-white/80 text-red-700 hover:bg-red-50" onClick={goNextSlide} disabled={!slides.length || safeSlideIndex === slides.length - 1}>
              ...
              <Card className="rounded-2xl border border-red-100 bg-white/95 shadow-md backdrop-blur">
                <CardContent className="p-8 md:p-10">
                  <div className="grid gap-6 md:grid-cols-3">
                    <div className="rounded-2xl border border-red-100 bg-gradient-to-br from-white to-red-50 p-5 shadow-sm">
                      <div className="text-sm text-muted-foreground">Règle 1</div>
                      <div className="mt-2 text-lg font-bold">Domicile</div>
                      <div className="mt-2 text-sm">Les matchs à domicile sont triés par heure.</div>
                    </div>

                    <div className="rounded-2xl border border-red-100 bg-gradient-to-br from-white to-red-50 p-5 shadow-sm">
                      <div className="text-sm text-muted-foreground">Règle 2</div>
                      <div className="mt-2 text-lg font-bold">Extérieur</div>
                      <div className="mt-2 text-sm">Les matchs à l’extérieur sont triés par catégorie.</div>
                    </div>

                    <div className="rounded-2xl border border-red-100 bg-gradient-to-br from-white to-red-50 p-5 shadow-sm">
                      <div className="text-sm text-muted-foreground">Règle 3</div>
                      <div className="mt-2 text-lg font-bold">Ordre</div>
                      <div className="mt-2 text-sm">Masculin avant féminin quand il faut départager.</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <>
                <Card className="rounded-2xl border border-red-100 bg-white/95 shadow-md backdrop-blur">
                  ...
                  <CardContent className="text-sm text-muted-foreground">
                    L’aperçu suit maintenant l’ordre du post : intro puis une page par bloc de planning.
                  </CardContent>
                </Card>

                {showHome ? <Section title="Matchs à domicile" groups={homeGroups} home /> : null}
                {showAway ? <Section title="Matchs à l’extérieur" groups={awayGroups} home={false} /> : null}
              </>
            )}
          </div>
          ...
          <div className="xl:sticky xl:top-6">
            <div className="space-y-4">
              {activeSlide?.type === 'intro' ? (
                <div className="overflow-hidden rounded-[28px] border shadow-2xl bg-white">
                  <img key={introImage} src={introImage} alt="Page intro" className="w-full h-[820px] object-cover" />
                </div>
              ) : null}

              {activeSlide?.type === 'matches' ? (
                <div
                  className="min-h-[820px] overflow-hidden rounded-[28px] border shadow-2xl"
                  style={{ background: background ? `url(${background}) center/cover` : '#7f1d1d' }}
                >
                  <div className="min-h-[820px] bg-black/15 p-6 md:p-8">
                    <div className="mx-auto max-w-4xl space-y-6">
                      <div className="space-y-2 pt-2 text-center text-white">
                        <h2 className="text-3xl font-black tracking-wide md:text-5xl">{previewTitle || 'PLANNING DU WEEK-END'}</h2>
                        <div className="text-lg font-bold uppercase md:text-2xl">{activeSlide.day}</div>
                        <div className="text-base font-bold uppercase md:text-xl">{activeSlide.venue}</div>
                      </div>
                ...
                >
                  <div className="min-h-[820px] bg-black/15 p-6 md:p-8">
                    <div className="mx-auto max-w-4xl space-y-6">
                      <div className="space-y-2 pt-2 text-center text-white">
                        <h2 className="text-3xl font-black tracking-wide md:text-5xl">{previewTitle || 'PLANNING DU WEEK-END'}</h2>
                        <div className="text-lg font-bold uppercase md:text-2xl">{previewSubtitle || 'APERÇU EN DIRECT'}</div>
                      </div>
                      ...
                      <div className="rounded-3xl border border-dashed border-white/40 bg-black/10 p-10 text-center text-white/90">
                        ...
                        <div className="mt-3 text-sm md:text-base">
                          Ajoute une page intro et importe un fichier Excel pour voir les slides du post une par une.
                        </div>
                      </div>
                    </div>
                  </div>
